...
fprintf('   E=experimental perceptual task with one disk\n')
...
%%CHANGEHERE
if( ~strcmp(epar.GrTp,'PGME-LS') && ~strcmp(epar.GrTp,'PMGE-LS')...
        && ~strcmp(epar.GrTp,'MPGE-LS') && ~strcmp(epar.GrTp,'MGPE-LS')...
        && ~strcmp(epar.GrTp,'GPME-LS') && ~strcmp(epar.GrTp,'GMPE-LS')...
        && ~strcmp(epar.GrTp,'PGME-SL') && ~strcmp(epar.GrTp,'PMGE-SL')...
        && ~strcmp(epar.GrTp,'MPGE-SL') && ~strcmp(epar.GrTp,'MGPE-SL')...
        && ~strcmp(epar.GrTp,'GPME-SL') && ~strcmp(epar.GrTp,'GMPE-SL'))
  PrintUsage;
...
if(epar.Sub==999)
  ...
  if(strcmp(epar.Task,'Grasp')) 
      epar.BlTpVector=['G'];
  elseif(strcmp(epar.Task,'ManEst')) 
    epar.BlTpVector=['M']; 
  elseif(strcmp(epar.Task,'Perc')) 
      epar.BlTpVector=['P-L'
                       'P-S'];
  elseif(strcmp(epar.Task,'ExPerc')) 
      epar.BlTpVector=['E'];
  else
    error('Wrong task')
  end
else
    if(contains(epar.GrTp,'G') && contains(epar.GrTp,'M') && contains(epar.GrTp,'P') ...
            && contains(epar.GrTp,'E') && contains(epar.GrTp,'LS'))
        if(strcmp(epar.Task,'Grasp')) 
            epar.BlTpVector=['G-PracWT'
                             'G-PracNT'
                             'G-1     '
                             'G-2     '
                             'G-3     '
                             'G-4     '];
        elseif(strcmp(epar.Task,'ManEst')) 
            epar.BlTpVector=['M-PracWT' % with tutorial
                             'M-PracNT' % no tutorial
                             'M-1     '
                             'M-2     '
                             'M-3     '
                             'M-4     ']; 
        ...
        elseif(strcmp(epar.Task,'Perc')) 
            epar.BlTpVector=['P-L-Prac' % no different versions
                             'P-L1    '
                             'P-L2    '
                             'P-S-Prac'
                             'P-S1    '
    ...
    elseif(contains(epar.GrTp,'G') && contains(epar.GrTp,'M') && contains(epar.GrTp,'P') ...
            && contains(epar.GrTp,'E') && contains(epar.GrTp,'SL'))
        if(strcmp(epar.Task,'Grasp')) 
            epar.BlTpVector=['G-PracWT'
                             'G-PracNT'
                             'G-1     '
...
practrials = 2; % for Grasp and Manest, is taken x2 due to change in VE do 2 
...
% count the errors per task (P/G/M/E): Use containers. Map for a
...
% count approved FGAp values for Grasp and ManEst
epar.FGApCount = containers.Map('KeyType','char','ValueType','uint32'); 
epar.FGApCount('G') = 0;
epar.FGApCount('M') = 0;
...
elseif(contains(epar.Task,'Perc'))
  drawInstructions(epar.Task);
  fprintf('Please keep fingers on buttons...\n')
else
...
elseif(strcmp(epar.Task,'Perc') || strcmp(epar.Task,'ExPerc')) 
...
function epar=DoPercTrial(epar)
    ...
    % ExPerc or Perc trial?
    if(strcmp(epar.Task,'ExPerc')) 
        ExPerc=true;
    else
        ExPerc=false;
    end
       ...
       if(currstatus(epar.leftKeyIndex)==0||currstatus(epar.rightKeyIndex)==0)
            ...
            if ExPerc
                ...
                if(currstatus(epar.leftKeyIndex)==0)
                    epar.PercEst.Resp = 'SubjL';
                    if contains(epar.TrTp,'D2')
                        correctness = 'F';
                    elseif contains(epar.TrTp,'D1')
                        correctness = 'C';        
                    end
                    epar.PercEst.Corr = correctness;
                elseif(currstatus(epar.rightKeyIndex)==0)
                    epar.PercEst.Resp = 'SubjS'; 
                    if contains(epar.TrTp,'D2')
                        correctness = 'C';
                    elseif contains(epar.TrTp,'D1')
                        correctness = 'F';        
                ...
                if(currstatus(epar.leftKeyIndex)==0)
                    ...
                    elseif(contains(epar.BlTp,'L') && contains(epar.TrTp,'R:D1') && contains(epar.TrTp,'L:D2'))
                        ...
                        fprintf('This should not occur. Something very wrong!! Stop experiment and fix/ask for help!\n');               
                    end
                    epar.PercEst.Corr = correctness;
                elseif(currstatus(epar.rightKeyIndex)==0)
...
while(~SpoolComplete)
    ...
    if ((odataTimeAFTER-TimeatTrialStart) > epar.PresentationTime)
        closeGoggles(epar);
    end
...
% dummy fill to overwrite later
epar.approved.FGAp = '0';
...
if(~GraspError)
    ...
    % manually approve if FGAp should be considered for non-error trials
    if(a=='+')
        b=ptb_ask_and_get_answerchar('FGAp (pink line) usable? (+-) ','+-');
        if(b=='+') 
            block = extractBefore(epar.BlTp,'-');
            epar.FGApCount(block) = epar.FGApCount(block) + 1;
            epar.approved.FGAp = '1';
        end
    end
end
...
if(GraspError)
  ...
  if(~LeftDiscArea)           ,fprintf('  Did not leave disc area (in time)\n')    , end
...
% %Convert & save existing Optotrak data-files:
approved = epar.approved;
% save(SubFiles.MatFile, 'approved', '-append', '-mat');
save(SubFiles.MatFile, 'approved', '-append');
...
function savePercFiles(epar,SubFiles)
    GrTp =epar.GrTp;
    BlTp =epar.BlTp;
    TrTp =epar.TrTp;
...
if (strcmp(epar.Task,'Grasp'))
    fprintf('Current errors: %i\n', epar.errorCount('G'));
    fprintf('Current FGAps: %i\n',  epar.FGApCount('G'));
elseif (strcmp(epar.Task,'ManEst'))
    fprintf('Current errors: %i\n', epar.errorCount('M'));
    fprintf('Current FGAps: %i\n',  epar.FGApCount('M'));
elseif (strcmp(epar.Task,'Perc'))
