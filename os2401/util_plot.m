...
if isnumberVF(eval.MaxAp)
  hAp(5)=plot([eval.MaxApTime eval.MaxApTime],[0 eval.MaxAp],'m-');
  axis([eval.MinTime,eval.MaxTime,ApRange]);
end
if isnumberVF(eval.ManEstAp)
  hAp(5)=plot([eval.ManEstTime eval.ManEstTime],[0 eval.ManEstAp],'m-');
  axis([eval.MinTime,eval.MaxTime,MeRange]);
end
hAp( 6)=ablineV(eval.StartVFTime     ,'b-' );
hAp(9) =ablineV(eval.LeftDiscAreaTime      ,'g-' ); 
if(~strcmp(Task,'ManEst'))
...
hAp(2)=plot(eval.Time,ThumbMissing ,'b.-');
if isnumberVF(eval.MaxAp)
  hAp(3)=plot([eval.MaxApTime eval.MaxApTime],[0 eval.MaxAp],'m-');
  axis([eval.MinTime,eval.MaxTime,eval.MaxAp-ApSurplus,eval.MaxAp+ApSurplus]);
end
if isnumberVF(eval.ManEstAp)
  hAp(3)=plot([eval.ManEstTime eval.ManEstTime],[0 eval.ManEstAp],'m-');
...
if(~strcmp(Task,'ManEst'))
    hAV(3)=ablineV(eval.MaxApTime       ,'m-' );
else
    hAV(4)=ablineV(eval.ManEstTime      ,'m-' );
end
if(~strcmp(Task,'ManEst'))
